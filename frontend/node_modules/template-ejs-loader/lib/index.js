...
exports.htmlWebpackPluginTemplateCustomizer = void 0;
const fs_1 = require("fs");
const path_1 = require("path");
const ejs_1 = require("ejs");
const requireFunction = (requestPath) => {
    if (requestPath.endsWith('.json')) {
        return JSON.parse((0, fs_1.readFileSync)(requestPath, 'utf-8'));
    }
    else {
        return require(requestPath);
    }
};
const resolveRequirePaths = async (context, content) => {
...
const customStringify = (obj, objectName) => {
    ...
    Object.keys(jsonObj).forEach((o) => {
        if (typeof jsonObj[o] === 'function') {
            console.error(`${consoleColor}htmlWebpackPluginTemplateCustomizer => ${objectName}${o} is NOT supported, because wepack loader inline do not support options in which type is function.${consoleColorReset}`);
            delete jsonObj[o];
        }
    });
    return JSON.stringify(jsonObj);
};
function htmlWebpackPluginTemplateCustomizer(config) {
    const htmlLoader = require.resolve('html-loader'); // get html-loader entry path
    const templateEjsLoader = process.env.NODE_ENV === 'test' ? (0, path_1.resolve)(process.cwd(), 'lib/index.js') : require.resolve('template-ejs-loader'); // get template-ejs-loader entry path
...
exports.htmlWebpackPluginTemplateCustomizer = htmlWebpackPluginTemplateCustomizer;
async function ejsLoader(content, sourceMap, additionalData) {
    ...
    const customizeIncluder = (originalPath, parsedPath) => {
        let includerResult = { filename: parsedPath };
    ...
    const currentHtmlWebpackPlugin = this._compiler?.options.plugins.filter((plugin) => typeof plugin === 'object' &&
        ...
        plugin.options.template === this.resource)[0];
    ...
    const parameter = Object.assign({
        require: (source) => requireFunction(source),
    }, loaderOptions.data, templateParameters);
    try {
        const resolveContent = await resolveRequirePaths(this, content);
        const template = await (0, ejs_1.compile)(resolveContent, ejsOptions)(parameter);
